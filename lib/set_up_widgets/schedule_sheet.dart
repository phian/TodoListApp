...
class _ScheduleSheetState extends State<ScheduleSheet> {
  List<String> _scheduleTittle = ['Today', 'Tomorrow', 'This week', 'Later', 'Choose date'];
  ...
  void initState() {
    ...
    _scheduleChoseDateTime = widget.schedulePickedDate == DateTime.now() ? DateTime.now() : widget.schedulePickedDate;
    _scheduleChoseDateText = DateFormat("EEEEEEEE dd MMMM yyyy").format(_scheduleChoseDateTime);
                ...
                return Container(
                  ...
                  child: ListTile(
                    ...
                    trailing: (_selectedScheduleIndex == index && index == 0)
                        ? Text(DateFormat("EEEEEEEE dd MMMM yyyy").format(DateTime.now()))
                        : (_selectedScheduleIndex == index && index == 1)
                            ? Text(DateFormat("EEEEEEEE dd MMMM yyyy").format(DateTime.now().add(Duration(days: 1))))
                            : (_selectedScheduleIndex == index && index == _scheduleTittle.length - 1)
                      ...
                      if (index == _scheduleTittle.length - 1) {
                        showRoundedDatePicker(
                          ...
                          setState(() {
                            ...
                            if (DateFormat("dd MMMM yyyy").format(_scheduleChoseDateTime) ==
                                DateFormat("dd MMMM yyyy").format(DateTime.now())) {
                              _selectedScheduleIndex = 0;

                              return;
                            } else if (DateFormat("dd MMMM yyyy").format(_scheduleChoseDateTime) ==
                            ...
                            var temp =
                                _scheduleChoseDateTime.add(Duration(days: -(_scheduleChoseDateTime.weekday - 1)));
                            if (DateFormat("dd MMMM yyyy").format(_scheduleChoseDateTime) ==
                            ...
                            //------------------Phần xét Later----------------//
                            if (DateFormat("dd MMMM yyyy").format(_scheduleChoseDateTime) ==
                                    DateFormat("dd MMMM yyyy").format(DateTime.now().add(Duration(days: 15))) &&
                            ...
                            if (DateFormat("dd MMMM yyyy").format(_scheduleChoseDateTime) ==
                                    DateFormat("dd MMMM yyyy").format(DateTime.now().add(Duration(days: 14))) &&
                                DateTime.now().weekday != 7) {
                              _selectedScheduleIndex = 3;
                            }
                            //------------------------------------------------//
                        ...
                        setState(() {
                          _scheduleChoseDateTime =
                              index == 0 ? DateTime.now() : DateTime.now().add(Duration(days: 1));
                          _scheduleChoseDateText = DateFormat("EEEEEEEE dd MMMM yyyy").format(_scheduleChoseDateTime);
                        ...
                        setState(() {
                          ...
                          if (index == 2) {
                            ...
                            _scheduleChoseDateTime = _scheduleChoseDateTime.add(Duration(days: 6));
                            ...
                            if (DateTime.now().weekday == 7) {
                              ...
                              _scheduleChoseDateTime =
                                  DateTime.now().add(Duration(days: -(DateTime.now().weekday - 1)));
                              _scheduleChoseDateTime = _scheduleChoseDateTime.add(Duration(days: 14));
                              _scheduleChoseDateText =
                                  DateFormat("EEEEEEEE dd MMMM yyyy").format(_scheduleChoseDateTime);
                            }
  ...
  void _onRepeatTap() {
    showModalBottomSheet(backgroundColor: Colors.transparent, context: context, builder: (context) => _repeatSheet)
        .whenComplete(() {
  ...
  void _initPositionForScheduleChoice() {
    ...
    var temp = widget.schedulePickedDate.add(Duration(days: -(widget.schedulePickedDate.weekday - 1)));
